...
- `EMAIL_PROVIDER` (`smtp` or `resend` for real emails)
...
- `RESEND_API_KEY`, `EMAIL_FROM` (only if `EMAIL_PROVIDER=resend`)
...
- If you see SMTP timeouts (e.g. `ETIMEDOUT` to port `465`/`587`) on your hosting provider, switch to `EMAIL_PROVIDER=resend` (HTTPS API) instead of SMTP.
